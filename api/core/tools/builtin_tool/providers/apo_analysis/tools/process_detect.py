...
class TimeSeriersAnalysisTool(BuiltinTool):
    def _invoke(
        ...
        detect_name = tool_parameters.get("detectName")
        data = tool_parameters.get("data")
        history = tool_parameters.get("history")
        res = ""
        match detect_name:
            case "ebpf_detect":
                res = self.ebpf_metric_detect(data)
        yield self.create_text_message(res)

    def spike_detec(self, data: dict, history: dict):
        pass

    def ebpf_metric_detect(self, result_str):
